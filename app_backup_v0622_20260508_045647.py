...
st.header("🤖 V0.6.1 整机中文需求 → 多工站工程包")
...
if st.button("🤖 AI解析并生成 V0.6.1 多工站工程包", use_container_width=True):
    try:
        ...
        with st.spinner("正在生成 V0.6.1 多工站工程包..."):
        ...
        if not result_v061["ok"]:
            ...
            v061_config_path = output_dir / "multi_station_config_ai_v061.json"
            v061_config_path.write_text(v061_config_text, encoding="utf-8")

            st.success("V0.6.1 多工站工程包生成完成。")
            ...
            st.download_button(
                ...
                data=zip_bytes_v061,
                file_name="AI_Device_Project_Package_V061.zip",
                mime="application/zip"
            )
            ...
            st.download_button(
                ...
                file_name="multi_station_config_ai_v061.json",
                mime="application/json"
            )
