...
CHECK_SCRIPT = PROJECT_DIR / "check_v065_product_console.py"
START_BAT = PROJECT_DIR / "start_v065_product_console.bat"
...
def backup_file(path: Path):
    if path.exists():
        backup = path.with_name(
            f"{path.stem}_backup_v065_{datetime.now().strftime('%Y%m%d_%H%M%S')}{path.suffix}"
        )
        shutil.copy2(path, backup)
        print("✅ 备份：", backup)


def install_app_product():
...
'''
    write_file(APP_PRODUCT, content)
...
def install_start_bat():
    backup_file(START_BAT)
...
pause
'''
    START_BAT.write_text(textwrap.dedent(content).lstrip(), encoding="utf-8")
    print("✅ 写入：", START_BAT)


def main():
    print("========== 安装 V0.6.5 Product Console ==========")
    ...
    print("\n========== 安装结果 ==========")
    print("✅ V0.6.5 成品版操作台安装完成")
    print("")
    print("下一步运行：")
    print("python check_v065_product_console.py")
    print("")
    print("验收通过后启动：")
    print("streamlit run app_product.py")
    print("")
    print("或者双击：")
    print("start_v065_product_console.bat")
