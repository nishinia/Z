...
def extract_json(text: str) -> dict:
    """
    从 AI 输出里提取 JSON，避免模型带 Markdown。
    """
    text = text.strip()
...
def parse_project_info_from_text(requirement: str) -> dict:
    ...
    description = requirement.strip().splitlines()[0] if requirement.strip() else description
...
def parse_flow_actions_from_text(requirement: str) -> list[dict]:
    """
    从流程文字里抽取 CYx 到动点 / 回原点。
    示例：
    CY1拨料上下气缸先到动点；
    CY2 NG剔除气缸到动点；
    CY2回原点；
    ...
    for match in pattern.finditer(flow_text):
        cy = match.group(1).strip().upper()
...
def build_station_config_from_text(requirement: str, cylinders: list[dict], ai_station_config: dict | None = None) -> dict:
    station_info = parse_station_info_from_text(requirement)
    flow_actions = parse_flow_actions_from_text(requirement)
...
def normalize_device_config(config: dict, requirement: str = "") -> dict:
    """
    给 AI 解析结果补默认字段，并用 Python 从原文强制修正关键字段。
    """
    ...
    # 气缸：如果正则能解析，优先用正则结果，避免 AI 用“气缸描述”
...
【要求】
1. 必须保留用户输入中的真实气缸名称，例如“拨料上下气缸”“NG剔除气缸”。
